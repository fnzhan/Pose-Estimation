...
def main():
    ...
    with torch.no_grad():
        for itr, (input_img, bbox) in enumerate(tqdm(tester.batch_generator)):
            # forward
            target = {}
            target['bbox'] = bbox
            coord_out = tester.model(input_img, target)
            coord_out = coord_out.view(-1, 18, 3)

            if cfg.flip_test:
                flipped_input_img = flip(input_img, dims=3)
                flipped_coord_out = tester.model(flipped_input_img, target)
                flipped_coord_out = flipped_coord_out.view(-1, 18, 3)
                flipped_coord_out[:, :, 0] = -flipped_coord_out[:, :, 0]
            ...
            if vis:
                ...
                tmpkps[:2, :] = coord_out[0, :, :2].cpu().numpy().transpose(1, 0) / cfg.output_shape[0] * \
                                cfg.input_shape[0]
                tmpkps[2, :] = 1
            ...
            # coord_out = torch.randn(input_img.shape[0],18,3)
